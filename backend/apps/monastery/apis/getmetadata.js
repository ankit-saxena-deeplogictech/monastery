...
exports.doService = async jsonReq => {
  if (!validateRequest(jsonReq)) { LOG.error(`Bad Request`); return { result: false }; }
    ...
    if (userslist.result && userslist.users.length > 0) {
      ...
      if(!fs.existsSync(`${APP_CONSTANTS.META_DIR}/${jsonReq.org}/${jsonReq.name.toLowerCase()}_${jsonReq.server}_${jsonReq.port}.json`)) return { result: false };
...
function _getMetadata(jsonReq) {
  let filedata = fs.readFileSync(`${APP_CONSTANTS.META_DIR}/${jsonReq.org}/${jsonReq.name.toLowerCase()}_${jsonReq.server}_${jsonReq.port}.json`);
