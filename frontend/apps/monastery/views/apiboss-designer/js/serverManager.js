...
async function setDefaultSettings(org,userid,server,port,packageName,apikey,isPublic) {
    apiman.registerAPIKeys({"*":"fheiwu98237hjief8923ydewjidw834284hwqdnejwr79389"},"X-API-Key");

    try {   // try to publish now
        return {result: (await apiman.rest(APP_CONSTANTS.API_SETDEFAULTSETTINGS, "POST", 
            { org,id:userid,server,port,package:packageName,apikey,isPublic}, true,true)).result, err: "Setting default server failed at the server", 
